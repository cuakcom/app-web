...
define('PRE_UPGRADE_SCRIPT_VERSION', '18.0.52.0'); //script version
define('PLESK_VERSION', '18.0.52'); // latest Plesk version
...
class Plesk17KnownIssues
{
    function validate()
    {
        ...
        if (Util::isLinux()) {
            ...
            $this->_checkAbilityToChmodInDumpd(); //:INFO: ERROR while trying to backup MySQL database. https://support.plesk.com/hc/en-us/articles/360010401359
    ...
    //:INFO: ERROR while trying to backup MySQL database. https://support.plesk.com/hc/en-us/articles/360010401359
        ...
        if (false === file_put_contents($file, 'test')) {
            ...
            if (!$result) {
                $log->emergency(
                    ...
                    . 'The upgrade procedure will fail. Please refer to https://support.plesk.com/hc/articles/360010401359 for details.'
                );
                $log->resultError();
                return;
            }
