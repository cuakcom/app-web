...
has_rh_enabled_repo()
{
	...
	case "$package_manager" in
		yum) yum repolist enabled --verbose --cacheonly -q | egrep -q "^Repo-id\s*: $repo\s*$" ;;
		# note: --noplugins may cause failure and empty output on RedHat
		dnf) dnf repoinfo --enabled --cacheonly -q | egrep -q "^Repo-id\s*: $repo\s*$" ;;
	esac
}
...
check_repos_rhel7()
{
	...
	! has_rh_enabled_repo "epel/$os_arch" || return 0
