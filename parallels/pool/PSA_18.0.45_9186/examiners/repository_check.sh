...
check_repos_centos8()
{
	...
	for repo in "PowerTools"; do
		...
		! has_rh_enabled_repo "$repo_lc" || continue
...
check_repos_cloudlinux8()
{
	...
	dnf config-manager --set-enabled "powertools" && has_rh_enabled_repo "powertools" && return 0
	dnf config-manager --set-enabled "cloudlinux-PowerTools" && has_rh_enabled_repo "cloudlinux-PowerTools" && return 0
...
check_repos_rhel7()
{
	...
	yum install --disablerepo 'PLESK_*' -q -y 'epel-release' 2>/dev/null \
		|| yum install --enablerepo 'PLESK_18_*-thirdparty' -q -y 'epel-release' \
		&& yum update -q -y 'epel-release'
